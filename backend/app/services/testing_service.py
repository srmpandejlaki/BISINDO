...
class TestingService:
    ...
    async def realtime_inference_websocket(self, websocket, idTraining: int, db: Session):
        ...
        if not training:
            await websocket.close(code=4004, reason="Model training record tidak ditemukan")
            return
            
        if not training.trainModelPath or not os.path.exists(training.trainModelPath):
            await websocket.close(code=4004, reason="Berkas model tidak ditemukan di server")
            return
            
        # 2. Get dataset and labels
        dataset = db.query(Dataset).filter(Dataset.idDataset == training.idDataset).first()
        labels = []
        ...
        # 3. Load model and get sequence length
        K.clear_session()
        try:
            model = load_model(training.trainModelPath)
        except Exception as e:
            await websocket.close(code=5000, reason=f"Gagal memuat model: {str(e)}")
            return
        ...
        try:
            while True:
                ...
                if frame is None:
                    await websocket.send_json({"success": False, "error": "Gagal decode gambar"})
                    continue
                ...
                if len(sequence) == sequence_length:
                    # Normalization
                    normalized_sequence = []
                    for f_data in sequence:
                        f_data = f_data.copy()
                        left_hand = f_data[:63].reshape(21, 3)
                        right_hand = f_data[63:].reshape(21, 3)
                    ...
                    # Predict
                    X_input = np.expand_dims(np.array(normalized_sequence, dtype=np.float32), axis=0)
